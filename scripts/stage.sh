...
case "${pkg}" in
    ...
    armada-splash|armada-rgb|fex|mesa|mangohud|gamescope|gamescope-session|gamescope-session-steam|kwin|plasma-mobile|powerdevil|inputplumber|networkmanager|jupiter-hw-support|umtp-responder|hexagonrpc|msm-firmware-loader) mkdir -p ctx/rpms && cp "${pkg}"/out/*.rpm ctx/rpms/ ;;
