...
case "${pkg}" in
    ...
    armada-splash|armada-rgb|armada-bottom-touchpads|fex|mesa|mangohud|gamescope|gamescope-session|gamescope-session-steam|kwin|plasma-mobile|powerdevil|inputplumber|networkmanager|jupiter-hw-support|umtp-responder) mkdir -p ctx/rpms && cp "${pkg}"/out/*.rpm ctx/rpms/ ;;
